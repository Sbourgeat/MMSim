...
@dataclass
class MarketParams:
    intial_price: float = 150.0
    volatility: float = 0.02
    drift: float = 0.0001
    market_hours: bool = True
    base_volume: int = 5*10**7
    volume_std: float = 0.05
    jump_intesity: float = 0.05
    jump_size_mean: float = 0.0
    jump_size_std: float = 0.02


class MarketSimulator:
    def __init__(self, params:None | [MarketParams] = None):
    ...
    def generate_price_path(self, n_days: int, n_intraday: int = 390) -> np.ndarray:
        ...
        # Combine components
        price_path = np.exp((self.params.drift - 0.5 * self.params.volatility**2) * dt + 
                          self.params.volatility * brownian_increments + jumps)
    ...
    def generate_volume_series(self, n_days: int, n_intraday: int = 390) -> np.ndarray:
        ...
        # Combine components
        volume_series = base_volume_per_step * intraday_pattern * volume_noise
    ...
    def _generate_jumps(self, n_steps: int, dt: float) -> np.ndarray:
        ...
        if n_jumps > 0:
            jump_sizes = self.rng.normal(self.params.jump_size_mean, 
                                      self.params.jump_size_std, n_jumps)
    ...
    def _apply_market_hours(self, data: np.ndarray, n_days: int, n_intraday: int) -> np.ndarray:
    ...
    def calibrate_to_historical(self, historical_data: pd.DataFrame) -> Dict[str, float]:
        """
        ...
        calibrated_params = {
            ...
            'volume_std': float(historical_data['Volume'].std() / historical_data['Volume'].mean())
        }
        
        return calibrated_params

# Utility functions for testing and examples
def create_sample_market_data(n_days: int = 252, use_aapl_params: bool = True) -> pd.DataFrame:
    """
...
def validate_market_data(df: pd.DataFrame) -> Dict[str, Any]:
    ...
    # Basic checks
    validation_results['has_ohlcv'] = all(col in df.columns for col in ['Open', 'High', 'Low', 'Close', 'Volume'])
    validation_results['no_nan_values'] = not df.isnull().any().any()
    validation_results['positive_prices'] = (df[['Open', 'High', 'Low', 'Close']] > 0).all().all().all()
    validation_results['positive_volume'] = (df['Volume'] > 0).all()
    
    # Realism checks
    validation_results['high_low_consistent'] = (df['High'] >= df['Low']).all()
    validation_results['close_in_range'] = ((df['Close'] >= df['Low']) & (df['Close'] <= df['High'])).all()
    validation_results['open_in_range'] = ((df['Open'] >= df['Low']) & (df['Open'] <= df['High'])).all()
    
    # Statistical checks
    returns = df['Close'].pct_change().dropna()
    validation_results['return_volatility'] = float(returns.std())
    validation_results['price_volume_correlation'] = float(abs(df['Close'].corr(df['Volume'])))
...
if __name__ == "__main__":
    ...
    data = simulator.create_ohlcv_data(n_days=30, n_intraday=78)  # 30 days, 5-minute bars
